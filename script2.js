// This script is to pass the response according to genre
            ...
            if(responseText) {
                ...
                document.querySelectorAll('.checkbox').forEach(element => {
                    element.addEventListener('click', e => {
                        console.log('inside clicl');
                        const movieType = e.target.getAttribute('genre');
                        console.log(movieType);
                        renderList(movieType);
                    });
                });
                const renderList = (movieType) => {
                    Object.entries(data.movies).forEach(([keys, value]) => {
                        for (var i = 0; i < value.genres.length; i++) {
                            if (value.genres[i] == movieType) {
                                templateMovieList.content.innerHTML = "";
                                build(keys, value);
                            }
                            else {
                                document.querySelector('.all-movies').createElement(h1);
                                document.querySelector('.all-movies').querySelector('h1').innerHTML = 'No movies available';
                            }
                        }
                    });
                }

                const build = (keys, value) => {
